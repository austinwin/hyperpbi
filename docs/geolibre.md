...
`geolibre:build` verifies both upstream version and commit, installs the submodule lockfile, runs GeoLibre’s lite browser build with `GEOLIBRE_APP_BASE=/geolibre/`, configures an explicit enhanced-embed origin allowlist, copies the locked profile, and writes a build manifest. `geolibre:build:cached` skips the submodule install for a validated local cache.
