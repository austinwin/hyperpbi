...
describe("Power BI map bindings", () => {
    it("preserves Values fields and infers coordinate semantics without fixed map roles", () => {
        const columns = [
            { displayName: "Latitude", roles: { values: true }, type: { numeric: true } },
            { displayName: "Longitude", roles: { values: true }, type: { numeric: true } }
        ] as powerbi.DataViewMetadataColumn[];
        const dictionary = buildFieldDictionary(columns);
        expect(dictionary.fields.latitude).toMatchObject({ roles: ["values"], type: "latitude" });
        expect(dictionary.fields.longitude).toMatchObject({ roles: ["values"], type: "longitude" });
    });
    ...
    it("keeps every Values field available through the DataView pipeline", () => {
        const columns = [
            { displayName: "Asset", roles: { values: true }, type: { text: true } },
            { displayName: "Latitude", roles: { values: true }, type: { numeric: true } },
            { displayName: "Longitude", roles: { values: true }, type: { numeric: true } },
            { displayName: "Layer", roles: { values: true }, type: { text: true } }
        ] as powerbi.DataViewMetadataColumn[];
        ...
        expect(Object.keys(normalized.fields)).toEqual(["asset", "latitude", "longitude", "layer"]);
        expect(normalized.map.mode).toBe("latLon"); expect(normalized.map.layers[0].name).toBe("Map data"); expect(normalized.map.layers[0].features[0].properties.Asset).toBe("A-1");
    });

    it("normalizes lat/lon, layers, color, size, tooltips, and details", () => {
        const fields = {
            lat: field("lat", "Latitude", "values", "latitude"), lon: field("lon", "Longitude", "values", "longitude"),
            layer: field("layer", "Layer", "values"), status: field("status", "Status", "values"), size: field("size", "Risk Score", "values", "measure"),
            asset: field("asset", "Asset ID", "values"), address: field("address", "Address", "values")
        };
        ...
        const map = normalizeMapBindings(rows, fields, { latitude: "lat", longitude: "lon", layer: "layer", color: "status", size: "size", tooltip: ["asset"], details: ["address"] });
    ...
    it("uses geometry before coordinates and rejects invalid geometry safely", () => {
        const fields = { geometry: field("geometry", "Geometry", "values", "geometry"), lat: field("lat", "Latitude", "values", "latitude"), lon: field("lon", "Longitude", "values", "longitude") };
    ...
    it("supports EPSG:4326 X/Y and privacy-safe address mode", () => {
        const xyFields = { x: field("x", "X", "values", "measure"), y: field("y", "Y", "values", "measure") };
        expect(normalizeMapBindings([{ x: -87.63, y: 41.88 }], xyFields, { x: "x", y: "y" }).layers[0].features[0]).toMatchObject({ x: -87.63, y: 41.88, lat: 41.88, lon: -87.63 });
        const addressFields = { address: field("address", "Address", "values"), city: field("city", "City", "values") };
        const addressMap = normalizeMapBindings([{ address: "1 Main St", city: "Chicago" }], addressFields, { address: "address", city: "city" });
    ...
    it("parses supported geometry and resolves popup tokens without expressions", () => {
        ...
        const feature = normalizeMapBindings([{ asset: "MH001" }], { asset: field("asset", "Asset ID", "values") }, { address: "asset" }).layers[0].features[0];
