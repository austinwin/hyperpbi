...
describe("AI authoring",()=>{
    it("builds a JSON-only, field-aware and JavaScript-free instruction",()=>{const prompt=buildAiPrompt(data,{...defaultAiPromptSettings,privacyMode:"summary",calculationsRequired:true},{width:1200,height:700});expect(prompt).toContain("risk_score");expect(prompt).toContain("Return only one valid JSON object");expect(prompt).toContain("does not execute JavaScript");expect(prompt).toContain("1200 x 700");expect(prompt).toContain("repeat.distinctBy");expect(prompt).toContain("valueFromRow");expect(prompt).toContain("Never invent externalSelection");});
