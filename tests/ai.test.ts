...
describe("AI authoring",()=>{
    it("builds a professional catalog- and recipe-aware instruction",()=>{const prompt=buildAiPrompt(data,{...defaultAiPromptSettings,privacyMode:"summary",calculationsRequired:true},{width:1200,height:700});for(const text of ["risk_score","Return only one valid JSON object","does not execute JavaScript","1200 x 700","repeat.distinctBy","valueFromRow","Never invent externalSelection","Shared component properties","advancedChart","Executive overview","Map command center","Dense 600x500 Power BI visual","styles.globalCss","styles.components"])expect(prompt).toContain(text);});
