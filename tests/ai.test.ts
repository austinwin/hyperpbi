...
describe("AI authoring",()=>{
    ...
    it("unwraps a schema 2.0 specification and independent Runtime Config package",()=>{const result=extractJsonFromAiResponse('{"specification":{"version":"2.0","components":[]},"config":{"version":"1.0"}}');expect(result.value).toMatchObject({version:"2.0",components:[]});expect(result.config).toMatchObject({version:"1.0"});});
    ...
    it("builds a targeted repair prompt with warnings and strict output",()=>{const prompt=buildRepairPrompt("{}",["components is required"],data,["selectionTarget is not used"]);expect(prompt).toContain("components is required");expect(prompt).toContain("selectionTarget is not used");expect(prompt).toContain("Return one complete corrected JSON object only");expect(prompt).toContain("Valid Field Manifest aliases");expect(prompt).toContain('result must use version "2.0"');});
});
