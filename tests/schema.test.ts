...
describe("schema validation", () => {
    ...
    it("canonicalizes tab child aliases exactly once during compilation",()=>{for(const alias of ["children","components","content"]){const input={version:"2.0",components:[{type:"tabs",id:`tabs-${alias}`,tabs:[{id:"one",title:"One",[alias]:[{type:"text",id:`child-${alias}`,text:"Ready"}]}]}]};const definitions=expandDefinitions(input).value as any;const expanded=expandPatterns(definitions).value as any;expect(expanded.components[0].tabs[0].children).toHaveLength(1);expect(expanded.components[0].tabs[0].components).toBeUndefined();expect(expanded.components[0].tabs[0].content).toBeUndefined();expect(validateSchema(expanded).errors).toEqual([]);}});
