...
function context() {
    ...
    const value: RenderContextValue = { data, rows: rows, sourceRows: rows, sourceRowKeys: data.rowKeys, getRowsForComponent:()=>rows, componentRows:id=>componentRows(id,value), schema: { version: "2.0", components: [] }, settings: toRuntimeSettings(new VisualFormattingSettingsModel()), state: initialDashboardState(), dispatch: action => {value.state=dashboardReducer(value.state,action);}, warnings: [], selectExternal, clearExternal,applyExternalFilter,clearExternalFilter, reportInteraction, config: defaultConfig, webAccessAvailable: false, executeUiAction: () => ({ success: true }), isOverlayOpen: () => false };
...
describe("safe row-level interactions", () => {
    ...
    it("renders individual sanitized clickable rows and resolves valueFromRow", () => {
        ...
        expect(test.applyExternalFilter).toHaveBeenCalledWith("leadby","=","Pranav",expect.objectContaining({ componentId: "leadby_toggle_filter", field: "leadby", matchedRowCount:2 }));expect(test.selectExternal).not.toHaveBeenCalled();
    ...
    it("passes multiSelect for explicit custom selection interactions",()=>{const test=context();runSafeInteraction({action:"selectWhere",selectionMode:"toggle",external:true,externalMode:"selection",internal:false,where:{op:"=",left:{field:"status"},right:{value:"Open"}}},test.value,undefined,undefined,{componentId:"status_toggle",multiSelect:true});expect(test.selectExternal).toHaveBeenCalledWith([0,1],true,expect.objectContaining({componentId:"status_toggle",field:"status"}));});
    it("returns no matches for unknown clicked-row fields", () => {
        ...
        expect(result.matchedRows).toEqual([]); expect(test.clearExternalFilter).toHaveBeenCalled();expect(test.selectExternal).not.toHaveBeenCalled();
    });
});

describe("external control interactions",()=>{
    const settle=()=>new Promise<void>(resolve=>setTimeout(resolve,25));
    it("sends select and segmented Power BI filters without requiring internal filtering",async()=>{for(const type of ["select","segmentedControl"] as const){const test=context();const host=document.createElement("div");act(()=>render(h(RenderContext.Provider,{value:test.value},h(ControlBlock,{component:{type,id:type,field:"status",interaction:{enabled:true,internalMode:"none",externalMode:"filter"}}})),host));await act(settle);await act(async()=>{if(type==="select"){const select=host.querySelector("select") as HTMLSelectElement;select.value="Open";select.dispatchEvent(new Event("change",{bubbles:true}));}else{Array.from(host.querySelectorAll("button")).find(button=>button.textContent==="Open")?.dispatchEvent(new MouseEvent("click",{bubbles:true}));}await settle();});expect(test.applyExternalFilter).toHaveBeenCalledWith("status","=","Open",expect.objectContaining({componentId:type,componentType:type,matchedRowCount:2}));expect(test.selectExternal).not.toHaveBeenCalled();render(null,host);}});
    it("sends one In filter for multiSelect",async()=>{const test=context();const host=document.createElement("div");act(()=>render(h(RenderContext.Provider,{value:test.value},h(ControlBlock,{component:{type:"multiSelect",id:"statuses",field:"status",interaction:{enabled:true,internalMode:"none",externalMode:"filter"}}})),host));await act(settle);await act(async()=>{const select=host.querySelector("select") as HTMLSelectElement;Array.from(select.options).forEach(option=>option.selected=option.value==="Open"||option.value==="Closed");select.dispatchEvent(new Event("change",{bubbles:true}));await settle();});expect(test.applyExternalFilter).toHaveBeenCalledWith("status","in",["Closed","Open"],expect.objectContaining({componentId:"statuses",matchedRowCount:3}));expect(test.selectExternal).not.toHaveBeenCalled();render(null,host);});
    it("clears the external filter when an external text input is cleared",async()=>{const test=context();const host=document.createElement("div");act(()=>render(h(RenderContext.Provider,{value:test.value},h(ControlBlock,{component:{type:"textInput",id:"status_search",field:"status",interaction:{enabled:true,internalMode:"none",externalMode:"filter"}}})),host));await act(settle);const input=host.querySelector("input") as HTMLInputElement;await act(async()=>{input.value="Open";input.dispatchEvent(new Event("input",{bubbles:true}));await settle();});expect(test.applyExternalFilter).toHaveBeenLastCalledWith("status","contains","Open",expect.objectContaining({matchedRowCount:2}));test.clearExternalFilter.mockClear();await act(async()=>{input.value="";input.dispatchEvent(new Event("input",{bubbles:true}));await settle();});expect(test.clearExternalFilter).toHaveBeenCalledWith(expect.objectContaining({componentId:"status_search"}));render(null,host);});
});

describe("built-in table and diagnostics", () => {
    it("sends a table row's source index and keeps highlight rows visible", () => {
        const test = context(); const host = document.createElement("div"); const component: TableComponent = { type: "table", id: "details", interaction: { enabled: true, internalMode: "highlight", externalMode: "selection", showSelector: true }, columns: ["leadby"], search: false, pagination: false };
