...
const rows=[{asset_id:"MH001"}];const fields={asset_id:{key:"asset_id",displayName:"Asset ID",type:"dimension" as const,roles:[]}};const data:NormalizedData={rows,rowKeys:rows.map((_,i)=>`row-${i}`),fields,aggregates:calculateAggregates(rows),calculatedMetrics:{high_risk_count:3},map:normalizeMapBindings(rows,fields,undefined,undefined,rows.map((_,i)=>`row-${i}`))};
