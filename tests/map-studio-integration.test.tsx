...
describe("Map Studio and Visual Inspector integration", () => {
    it("opens the selected map on shared canonical JSON and selection state", () => {
        ...
        act(() => host.querySelector<HTMLButtonElement>('[data-workspace-trigger="Create"]')!.click());
        act(() => Array.from(host.querySelectorAll<HTMLButtonElement>('[role="menuitem"]')).find(item => item.textContent?.includes("Visual Inspector"))!.click());
