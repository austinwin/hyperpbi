...
import type { ResolvedMapRenderer, ResolvedMapSymbol, ResolvedMapFeature } from "../model/resolvedMapTypes";
import { resolvedFeatureValue, featureNumericDomain } from "../model/mapFeatureValue";
...
function clusterStyle(
    ...
    return {
        ...DEFAULT_STYLE,
        radius: renderer.clusterRadius ?? 10,
...
export function featureStyleWithDomain(
    ...
    if (renderer.type === "continuousColor") {
        ...
        const rawValue = resolvedFeatureValue(feature, field, renderer.fieldSource ?? "joined");
        const num = Number(rawValue);
        if (isNaN(num) || !isFinite(num)) return base;

        const [dMin, dMax] = domain;
        const t = dMax === dMin ? 0.5 : clamp((num - dMin) / (dMax - dMin), 0, 1);
        const color = interpolateColor(renderer.minColor ?? "#dbeafe", renderer.maxColor ?? "#2563eb", t);
