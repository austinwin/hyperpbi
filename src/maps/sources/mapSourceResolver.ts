...
const issue = (
  code: MapLayerDiagnosticIssue["code"],
  severity: MapLayerDiagnosticIssue["severity"],
  message: string,
  path?: string,
  details?: Record<string, unknown>,
): MapLayerDiagnosticIssue => ({ code, severity, message, path, details });
const bounded = <T>(values: T[], limit = 12) => values.slice(0, limit);

export function effectiveMapLayerDataset(
  layer: Pick<MapLayerDefinition, "dataset">,
  mapDataset?: string,
): string {
...
export function resolvePowerBiLayer(
  ...
  const requirePowerBiSource = (
    sourceName: "powerbi" | "service" | "joined",
    path: string,
  ): boolean => {
    if (sourceName === "powerbi") return true;
    issues.push(
      issue(
        "MAP_FIELD_SOURCE_UNAVAILABLE",
        "error",
        `Attribute source “${sourceName}” is unavailable for Power BI layer “${layer.name}”.`,
        path,
        { source: sourceName },
      ),
    );
  ...
  for (const [name, reference] of Object.entries(configured)) {
    ...
    if (typeof reference === "string" && !context.fields[reference])
      issues.push(
        issue(
    ...
    if (
      ["latitude", "longitude", "x", "y", "size"].includes(name) &&
    ...
    if (!field) {
      issues.push(
        issue(
          "MAP_LAYER_FIELD_NOT_FOUND",
          "error",
          `Field “${reference}” is not in dataset “${datasetName}”.`,
          path,
          { field: reference, dataset: datasetName },
        ),
      );
    ...
    if (
      ...
      issues.push(
        issue(
          "MAP_LAYER_NUMERIC_FIELD_REQUIRED",
          "error",
          `Field “${reference}” must be numeric, but it is ${field.dataType}.`,
          path,
  ...
  if (
    renderer &&
    "field" in renderer &&
    requirePowerBiSource(
      renderer.fieldSource ?? "powerbi",
      `${layerPath}/renderer/fieldSource`,
    )
  )
    validateField(
      renderer.field,
      `${layerPath}/renderer/field`,
      ["classBreaks", "continuousColor", "proportionalSize"].includes(
        renderer.type,
      ) ||
        (renderer.type === "densityGrid" &&
          ["sum", "avg"].includes(renderer.statistic ?? "count")),
    );
  if (
    ...
    validateField(
      ...
      `${layerPath}/renderer/weightField`,
      true,
    );
  if (
    ...
    validateField(
      renderer.aggregateField,
      `${layerPath}/renderer/aggregateField`,
      renderer.clusterLabel === "sum",
    );
  if (
    layer.labels &&
    requirePowerBiSource(
      layer.labels.fieldSource ?? "powerbi",
      `${layerPath}/labels/fieldSource`,
    )
  )
    validateField(layer.labels.field, `${layerPath}/labels/field`);
  layer.popup?.fields?.forEach((field, index) => {
    if (
      requirePowerBiSource(
        field.fieldSource ?? "powerbi",
        `${layerPath}/popup/fields/${index}/fieldSource`,
      )
    )
      validateField(field.field, `${layerPath}/popup/fields/${index}/field`);
  });
  layer.tooltip?.fields?.forEach((field, index) => {
    if (
      requirePowerBiSource(
        field.fieldSource ?? "powerbi",
        `${layerPath}/tooltip/fields/${index}/fieldSource`,
      )
    )
      validateField(field.field, `${layerPath}/tooltip/fields/${index}/field`);
  });
  validateField(layer.join?.powerBiField, `${layerPath}/join/powerBiField`);
  if (
    layer.visibility?.conditionField &&
    requirePowerBiSource(
      layer.visibility.conditionFieldSource ?? "powerbi",
      `${layerPath}/visibility/conditionFieldSource`,
    )
  )
    validateField(
      layer.visibility.conditionField,
      `${layerPath}/visibility/conditionField`,
    );
  (Array.isArray(layer.filter)
    ...
    if (
      requirePowerBiSource(
        filter.fieldSource ?? "powerbi",
        `${layerPath}/filter/${index}/fieldSource`,
      )
    )
      validateField(filter.field, `${layerPath}/filter/${index}/field`);
  });
  if (
    layer.interaction?.field &&
    requirePowerBiSource(
      layer.interaction.fieldSource ?? "powerbi",
      `${layerPath}/interaction/fieldSource`,
    )
  )
    validateField(layer.interaction.field, `${layerPath}/interaction/field`);
  if (
    layer.interaction?.externalMode === "filter" &&
    layer.interaction.field &&
    (layer.interaction.fieldSource ?? "powerbi") === "powerbi"
  ) {
    ...
    if (field && !externalFilterTargetFor(field))
      issues.push(
        issue(
          ...
          "error",
          `Field “${field.key}” has no Power BI model-column target for external filtering. Use selection/highlight or a group-by source field.`,
          `${layerPath}/interaction/field`,
          { field: field.key, dataset: datasetName, origin: field.origin },
        ),
      );
  }
  ...
  if (source.layerValue !== undefined) {
    const target = mapData.layers.find(
      (candidate) => candidate.name === source.layerValue,
    );
    layerValueMatched = Boolean(target);
    if (!target) {
      const availableValues = bounded(
        mapData.layers.map((candidate) => candidate.name),
      );
      ...
      return createEmptyLayer(
        ...
        [
          ...
          issue(
            ...
            "error",
            message,
            `${layerPath}/source/layerValue`,
            { requested: source.layerValue, availableValues },
          ),
        ],
        {
          resolvedLocationMode: mapData.mode,
          resolvedBindings: mapData.bindings as unknown as Record<
            string,
    ...
    normalizedFeatures = mapData.layers.flatMap(
  ...
  const filters = layer.filter
    ...
    : [];
  if (filters.length)
    normalizedFeatures = normalizedFeatures.filter((feature) =>
      filters.every(
        (filter) =>
          (filter.fieldSource ?? "powerbi") === "powerbi" &&
          matchesFilter(
            feature.row[filter.field],
            filter.operator,
            filter.value,
          ),
      ),
  ...
  if (
    layer.visibility?.conditionField &&
    layer.visibility.conditionValues?.length
  ) {
    const accepted = layer.visibility.conditionValues;
    normalizedFeatures = normalizedFeatures.filter(
      (feature) =>
        (layer.visibility!.conditionFieldSource ?? "powerbi") === "powerbi" &&
        accepted.some((value) =>
          Object.is(value, feature.row[layer.visibility!.conditionField!]),
        ),
    );
  }
  ...
  if (requestedCount > featureLimit) {
    ...
    issues.push(
      issue(
        ...
        "warning",
        `Layer “${layer.name}” was deterministically limited to ${featureLimit.toLocaleString()} of ${requestedCount.toLocaleString()} valid features.`,
        `${layerPath}/performance/maxFeatures`,
        { requestedCount, featureLimit },
      ),
    );
  }

  const features: ResolvedMapFeature[] = normalizedFeatures.map((feature) =>
    materializeFeature(feature, layer, context),
  );
  ...
  const resolvedRenderer = layer.renderer
    ? resolveRenderer(layer.renderer, features, "powerbi")
    : { type: "simple" as const, symbol: {} };
  const rendererCalculationMs =
    (globalThis.performance?.now?.() ?? Date.now()) - rendererStarted;
  ...
  return {
    ...
    labels: layer.labels
      ? {
          ...
          fieldSource: layer.labels.fieldSource ?? "powerbi",
    ...
    popup: layer.popup
      ? {
          ...
          fields: (layer.popup.fields ?? []).map((field) => ({
            ...field,
    ...
    diagnostics: {
      ...
      rendererFieldSource:
        layer.renderer && "fieldSource" in layer.renderer
          ? (layer.renderer.fieldSource ?? "powerbi")
          : undefined,
      labelFieldSource:
        layer.labels?.fieldSource ??
        (layer.labels?.field ? "powerbi" : undefined),
      sourceResolutionMs:
        (globalThis.performance?.now?.() ?? Date.now()) - started,
...
function materializeFeature(
  ...
  const sourceIndices =
    context.sourceRowIndices?.[logicalIndex]?.filter((index) => index >= 0) ??
    [context.rowIndices[logicalIndex] ?? logicalIndex].filter(
      (index) => index >= 0,
    );
...
export function geometryAnalysis(features: readonly ResolvedMapFeature[]): {
  type: MapGeometryType;
  counts: Partial<Record<MapGeometryType, number>>;
} {
  const counts: Partial<Record<MapGeometryType, number>> = {};
  for (const feature of features)
    counts[feature.geometryType] = (counts[feature.geometryType] ?? 0) + 1;
  const valid = (Object.keys(counts) as MapGeometryType[]).filter(
    (type) => type !== "unknown" && (counts[type] ?? 0) > 0,
  );
  return {
    type:
      valid.length === 0 ? "unknown" : valid.length === 1 ? valid[0] : "mixed",
    counts,
  };
}

function featureTypeToGeoType(
  type: string,
...
function createEmptyLayer(
  ...
  diagnostics: Partial<ResolvedMapLayer["diagnostics"]> = {},
): ResolvedMapLayer {
