...
function runDefinition(definition:DatasetDefinition,input:DatasetResult,name:string,sourceRowKeys:string[],schemaFields:Record<string,NormalizedField>,schemaWarnings:string[]):DatasetResult{
    ...
    const rows=items.map(item=>item.row);const lineage=items.map(item=>item.lineage);const rowKeys=lineage.map((indices,index)=>indices.length?indices.map(sourceIndex=>sourceRowKeys[sourceIndex]??String(sourceIndex)).join("|"):`dataset:${name}:${index}`);const fields=fieldsForRows(rows,schemaFields);
...
export function evaluateDatasets(base:NormalizedData,definitions:Record<string,DatasetDefinition>={},cache=new Map<string,DatasetResult>(),lineageOptions:{sourceIndices?:number[];sourceRowKeys?:string[]}={}):DatasetEvaluation{
    const schemaEvaluation=resolveDatasetSchemas(base,definitions);
    ...
    const resolve=(name:string):DatasetResult|undefined=>{if(datasets.has(name))return datasets.get(name);const definition=definitions[name];const schema=schemaEvaluation.datasets.get(name);if(!definition||!schema||visiting.has(name))return undefined;visiting.add(name);const input=definition.source==="powerbi"?baseResult:resolve(definition.source);visiting.delete(name);if(!input)return undefined;const signature=canonical({definition,input:input.signature,fields:schema.fields});const cached=cache.get(signature);if(cached){const hit={...cached,diagnostic:{...cached.diagnostic,cacheStatus:"hit" as const,evaluationMs:0}};datasets.set(name,hit);return hit;}const schemaWarnings=schemaEvaluation.diagnostics.filter(item=>item.path.startsWith(`/data/datasets/${name}/`)&&item.severity!=="info").map(item=>item.message);const result=runDefinition(definition,input,name,sourceRowKeys,schema.fields,schemaWarnings);cache.set(signature,result);datasets.set(name,result);return result;};
