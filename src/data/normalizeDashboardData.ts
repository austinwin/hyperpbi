...
export interface NormalizedDashboardData { fields: Record<string, NormalizedField>; rows: DataRow[]; schemaFromField?: string; }
...
export function normalizeDashboardData(dataView?: powerbi.DataView): NormalizedDashboardData {
    ...
    if (table?.columns?.length) {
        ...
        return { fields: dictionary.fields, rows, schemaFromField: schemaKey ? String(rows.find(row => row[schemaKey])?.[schemaKey] ?? "") : undefined };
    }
    ...
    return { fields: dictionary.fields, rows };
}
