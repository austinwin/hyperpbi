...
export function DataDisplayBlock({ component }: { component: DataDisplayComponent }) {
    ...
    const run=(event:Event)=>{const definition=payloadDefinition(component);const field=definition.field;const value=definition.value!==undefined?definition.value:field?aggregateValue(rows,field,definition.aggregation??"first"):undefined;const indices=field?(definition.aggregation&&definition.aggregation!=="first"?rows.map(row=>context.sourceRows.indexOf(row)).filter(index=>index>=0):context.sourceRows.map((row,index)=>row[field]===value?index:-1).filter(index=>index>=0)):[];if(!field&&value===undefined&&!indices.length){context.reportInteraction({componentId:id,componentType:component.type},"interaction payload unavailable");event.stopPropagation();return;}executeComponentInteraction(policy,createInteractionPayload(component,{rowIndices:indices,field,value}),context,{trigger:"click",event});};
    let content;
