...
export function DetailPanel({ component }: { component: DataDisplayComponent }) {
    const { rows, sourceRows, state, data }=useRenderContext();const selectedIndex=selectedSourceRowIndex(state);const row=component.selectedRow===true?(selectedIndex===undefined?undefined:sourceRows[selectedIndex]):rows[0];
