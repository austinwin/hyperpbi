...
export function SlotRenderer({ component, name, row }: { component: ComponentBase; name: keyof NonNullable<ComponentBase["slots"]>; row?: DataRow }) {
    const { data, state, schema, settings } = useRenderContext(); const template = component.slots?.[name]; if (!template) return null;
    ...
    const safe = sanitizeHtml(rendered, { allowInlineSvg: settings.allowInlineSvg, allowSafeImages: settings.allowSafeImages });
