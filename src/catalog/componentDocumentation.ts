...
export const componentDocs: Record<string, ComponentDocMeta> = {
    ...
    sankeyChart: { status: "stable", keyProperties: ["sourceField", "targetField", "valueField", "aggregation", "orientation"], supportsUiAction: false, supportsDataInteraction: true, accessibility: "Node and edge clicks retain distinct row bindings; accompany dense flows with a table when exact values matter." },
    ...
    svg: { status: "stable", keyProperties: ["viewBox", "elements", "dataContext", "motion", "performance", "css"], supportsUiAction: true, supportsDataInteraction: true, accessibility: "ariaLabel is recommended. Interactive marks are keyboard focusable and respond to Enter and Space. Motion respects the configured reduced-motion policy.", doNotUseWhen: "A standard analytical chart is available; use ECharts chart components for bars, lines, pies, scatterplots, heatmaps, Sankey, and treemaps.", related: ["svgMarkup", "gauge", "progressBar"] },
    svgMarkup: { status: "stable", statusNote: "Advanced escape hatch. Markup is parsed by the dedicated SVG sanitizer, isolated, and sanitized again before rendering.", keyProperties: ["svg", "viewBox", "ariaLabel", "description", "css"], supportsUiAction: false, supportsDataInteraction: false, accessibility: "The component wrapper supplies accessible image semantics; include title/desc content in the SVG where useful.", doNotUseWhen: "Declarative SVG can express the visual. Raw SVG cannot use scripts, events, foreignObject, external assets, use, image, style elements, or arbitrary URLs.", related: ["svg"] },
