...
function categoryChart(type: string, title: string, seriesOptions: JsonObject = {}): JsonObject {
    return {
        ...
        setOption: { notMerge: true, lazyUpdate: false, silent: false },
        options: {
            animation: true,
...
export const componentJsonExamples: Record<string, JsonObject> = {
    ...
    scatterChart: { ...base("scatterChart", "Measure relationship", 6), x: "__x_measure_field_key__", y: "__y_measure_field_key__", pointSize: "__size_measure_field_key__", height: 340, maxDataRows: 30000, initOptions: { renderer: "canvas", useDirtyRect: false }, setOption: { notMerge: true, lazyUpdate: false }, options: { tooltip: { trigger: "item" }, xAxis: { name: "X measure", scale: true }, yAxis: { name: "Y measure", scale: true }, series: [{ type: "scatter", emphasis: { focus: "series" } }] }, ...chartInteraction },
    ...
    sankeyChart: { ...base("sankeyChart", "Work order flow", 8), sourceField: "__source_field_key__", targetField: "__target_field_key__", valueField: measureField, aggregation: "sum", orientation: "horizontal", nodeAlign: "justify", height: 380, ...chartInteraction },
    ...
    svg: { ...base("svg", "Animated progress", 6), viewBox: "0 0 800 240", height: 240, ariaLabel: "Completion progress", description: "A data-bound progress track and value label.", dataContext: { mode: "aggregate" }, motion: { enabled: true, reducedMotion: "respect-system", maxConcurrentAnimations: 12 }, elements: [{ type: "defs", children: [{ type: "linearGradient", id: "progressGradient", x1: 0, y1: 0, x2: 1, y2: 0, children: [{ type: "stop", offset: "0%", stopColor: "var(--hp-primary)" }, { type: "stop", offset: "100%", stopColor: "var(--hp-success)" }] }] }, { type: "rect", id: "track", x: 40, y: 120, width: 720, height: 22, rx: 11, fill: "var(--hp-muted)" }, { type: "rect", id: "progress", x: 40, y: 120, width: { bind: measureField, scale: { type: "linear", domain: [0, 100], range: [0, 720], clamp: true } }, height: 22, rx: 11, fill: "url(#progressGradient)", animation: { preset: "progress-fill", durationMs: 1000, easing: "ease-out" } }, { type: "text", x: 40, y: 82, text: { template: `{{${measureField}}}%` }, fontSize: 28, fontWeight: 700 }] },
    svgMarkup: { ...base("svgMarkup", "Sanitized raw SVG", 6), viewBox: "0 0 400 160", height: 160, ariaLabel: "Operating status", description: "Advanced sanitized SVG markup example.", svg: `<svg viewBox="0 0 400 160" role="img"><defs><linearGradient id="statusGradient"><stop offset="0%" stop-color="#2563eb"/><stop offset="100%" stop-color="#16a34a"/></linearGradient></defs><rect id="statusTrack" x="20" y="50" width="360" height="40" rx="20" fill="url(#statusGradient)"/><text x="200" y="76" text-anchor="middle" fill="white">{{${field}}}</text></svg>`, css: ".hp-status { opacity: 1; }" },
