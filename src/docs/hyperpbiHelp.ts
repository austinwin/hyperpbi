...
Map Studio edits that same canonical JSON with a layer tree, provider/source controls, dataset-aware fields, renderers, labels, popup/tooltip configuration, join preview, structured filters, groups, bookmarks, performance limits, and diagnostics. Logical datasets produce filtered, derived, selected, or grouped layer views over the one flattened Power BI data view received by the visual; they never issue independent semantic-model queries. Grouped features retain all contributing source identities where Power BI supplied them.
