...
export function validateReferences(schema: HyperPbiSchema, data: NormalizedData): string[] {
    ...
    const visit = (component: DashboardComponent) => {
        ...
        if(component.type==="svg"){const svg=component as import("./hyperpbiSchema").SvgComponent;const scan=(value:unknown):void=>{if(!value||typeof value!=="object")return;if(Array.isArray(value)){value.forEach(scan);return;}const node=value as Record<string,unknown>;if(typeof node.bind==="string"&&!node.bind.startsWith("$")&&data.fields[node.bind])check(node.bind,owner);if(node.when&&typeof node.when==="object"&&typeof (node.when as Record<string,unknown>).field==="string"&&data.fields[String((node.when as Record<string,unknown>).field)])check(String((node.when as Record<string,unknown>).field),owner);Object.values(node).forEach(scan);};scan(svg.elements);}
