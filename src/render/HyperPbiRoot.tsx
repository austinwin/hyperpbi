...
export function HyperPbiRoot({ instanceId, schema, data, settings, renderMs, referenceWarnings = [], config = defaultConfig, selectExternal = notSent, clearExternal = notSent, reportInteraction = () => undefined, webAccessAvailable = false }: { instanceId: string; schema: HyperPbiSchema; data: NormalizedData; settings: RuntimeSettings; renderMs: number; referenceWarnings?: string[]; config?: HyperPbiConfig; selectExternal?: (rowIndices: number[], multiSelect?: boolean, details?: InteractionDetails) => ExternalSelectionResult; clearExternal?: (details?: InteractionDetails) => ExternalSelectionResult; reportInteraction?: (details: InteractionDetails, reason?: ExternalSelectionFailureReason, rowIndices?: number[]) => void; webAccessAvailable?: boolean }) {
    ...
    const gatedSelectExternal = (indices: number[], multiSelect = false, details: InteractionDetails = {}): ExternalSelectionResult => { if (config.interactions?.crossFilter === false) { reportInteraction(details, "interactions disabled", indices); return { sent: false, reason: "interactions disabled" }; } return selectExternal(indices, multiSelect, details); };
    const gatedClearExternal = (details: InteractionDetails = {}): ExternalSelectionResult => { if (config.interactions?.crossFilter === false) { reportInteraction(details, "interactions disabled"); return { sent: false, reason: "interactions disabled" }; } return clearExternal(details); };
    const context = useMemo(() => ({ data: filteredData, rows, sourceRows: data.rows, schema, settings, state, dispatch, warnings: sanitizedCss.warnings, selectExternal: gatedSelectExternal, clearExternal: gatedClearExternal, reportInteraction, config, webAccessAvailable }), [filteredData, rows, data.rows, schema, settings, state, sanitizedCss.warnings, selectExternal, clearExternal, reportInteraction, config, webAccessAvailable]);
