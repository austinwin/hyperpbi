...
export function createInteractionPayload(component: ComponentBase, options: {
    ...
    return {
        ...
        rowIndices: Array.from(new Set(options.rowIndices ?? [])).filter(index => Number.isInteger(index) && index >= 0),
        field,
        value,
...
export function deriveBoundPayload(component: ComponentBase, data: NormalizedData, sourceRows: DataRow[], value?: unknown): InteractionPayload {
    ...
    void data;
    return createInteractionPayload(component, { field, value: resolved, rowIndices });
}
